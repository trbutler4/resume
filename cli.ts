...
program
  .name('resume-gen')
  .description('TypeScript Resume Generator - Create tailored resumes for different job types')
  .version('1.0.0');
...
program
  ...
  .action((type: string, options) => {
    ...
    try {
      ...
      console.log(chalk.gray('🌐 Open in browser:'), chalk.cyan(`file://${path.resolve(outputFile)}`));
    } catch (error) {
...
program
  ...
  .action((options) => {
    ...
    console.log(chalk.yellow(`Generating ${availableTypes.length} resume types...\n`));
...
program
  ...
  .action(() => {
    ...
    console.log(chalk.gray('Usage:'), chalk.yellow(`resume-gen generate <type>`));
  });
...
program
  ...
  .action((options) => {
    ...
    try {
      ...
      const tscWatch = execSync('npm run dev > /dev/null 2>&1 &', { stdio: 'pipe' });
      ...
      console.log(chalk.green('🌐 Server running at:'), chalk.cyan(`http://localhost:${options.port}`));
...
program
  ...
  .action(() => {
    ...
    console.log(chalk.cyan('\nBuild Status:'), buildExists ? chalk.green('✓ Built') : chalk.red('✗ Not built'));
    
    if (!buildExists) {
      console.log(chalk.yellow('Run'), chalk.cyan('npm run build'), chalk.yellow('to build the project'));
    }
    
    // Check output directory
    const outputExists = fs.existsSync('resumes');
    console.log(chalk.cyan('Output Directory:'), outputExists ? chalk.green('✓ Exists') : chalk.gray('✗ Not created'));
    
    if (outputExists) {
      const files = fs.readdirSync('resumes').filter(f => f.endsWith('.html'));
...
program
  ...
  .action((options) => {
    ...
    if (!cleanBuild && !cleanResumes && !options.all) {
      console.log(chalk.yellow('No cleaning options specified. Use --help for options.'));
      return;
    }
...
program.on('command:*', () => {
  ...
  console.log(chalk.gray('See'), chalk.cyan('--help'), chalk.gray('for a list of available commands.'));
  process.exit(1);
});
