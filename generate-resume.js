...
class ResumeGenerator {
    ...
    generateWorkExperience() {
        if (!this.config.workExperience || !this.config.workExperience.include.length) {
            return '';
        }

        const experiences = this.config.workExperience.include
            .map(id => this.experienceData.workExperience.find(exp => exp.id === id))
            .filter(exp => exp)
            .sort((a, b) => {
    ...
    generateOpenSourceExperience() {
        if (!this.config.openSourceExperience || !this.config.openSourceExperience.include.length) {
            return '';
        }

        const experiences = this.config.openSourceExperience.include
            .map(id => this.experienceData.openSourceExperience.find(exp => exp.id === id))
            .filter(exp => exp)
            .sort((a, b) => {
    ...
    emphasizeText(text) {
        ...
        if (this.config.emphasize.keywords) {
            this.config.emphasize.keywords.forEach(keyword => {
                ...
                emphasizedText = emphasizedText.replace(regex, `<strong>${keyword}</strong>`);
            });
        }
        ...
        if (this.config.emphasize.metrics) {
            this.config.emphasize.metrics.forEach(metric => {
                if (emphasizedText.includes(metric)) {
                    emphasizedText = emphasizedText.replace(metric, `<strong>${metric}</strong>`);
                }
            });
        }
